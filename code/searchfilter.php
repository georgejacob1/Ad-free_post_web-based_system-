...
// $v = "SELECT * FROM (((`tbl_product` INNER JOIN tbl_subcat ON tbl_product.subcat_id=tbl_subcat.sub_id) INNER JOIN tbl_categories ON tbl_categories.cat_id=tbl_subcat.cat_id)INNER join tbl_users ON tbl_users.login_id=tbl_product.login_id) where p_name LIKE '%$search%' OR subcat LIKE '%$search%' OR category LIKE '%$search%' AND tbl_product.delete_status='1' ORDER BY tbl_product.p_name  ";
...
$_SESSION['range1'] = 0;
$_SESSION['range2'] = 30000000;
if (isset($_POST['filter'])) {
    $_SESSION['range1'] = $_POST['x'];
    $_SESSION['range2'] = $_POST['y'];
...
$sql2 = "SELECT * FROM tbl_address INNER JOIN tbl_product ON tbl_address.login_id=tbl_product.login_id INNER JOIN tbl_subcat ON tbl_product.subcat_id=tbl_subcat.sub_id INNER JOIN tbl_categories ON tbl_categories.cat_id=tbl_subcat.cat_id INNER JOIN tbl_users on tbl_product.login_id=tbl_users.login_id WHERE (tbl_address.street LIKE '%$lsearch%' OR tbl_address.city LIKE '%$lsearch%' OR tbl_address.state LIKE '%$lsearch%') AND (tbl_product.p_name LIKE '%$search%' OR tbl_categories.category LIKE '%$search%' OR tbl_subcat.subcat LIKE '%$search%') AND tbl_product.delete_status='1'";
...
<body>
    ...
    <?php
    $sqlpr = "SELECT MAX(price) AS highest_price FROM tbl_product";
    $resultpr = mysqli_query($conn, $sqlpr);

    // Fetch the result
    $rowpr = mysqli_fetch_assoc($resultpr);
    $highest_price = $rowpr['highest_price']; ?>
    ...
    <div>
        <div class="block-left">
            <div class="card">
                ...
                <br>
                <span>
                    <h2 style="margin-left: -180px;">Price Filter:-</h2>
                </span>
                <br>
                <br>
                <br>
                <div class="range-slider"><span>from
                    ...
                    <input value="100000" min="0" max="<?php echo $rowpr['highest_price']; ?>" step="500" type="range" />
                    <input value="100000" min="0" max="<?php echo $rowpr['highest_price']; ?>" step="500" type="range" />
            ...
            <section class="featured" id="featured">
                ...
                <div>
                    <ul class="mystyle-products" id="searched">
                        ...
                        while ($vrow =  mysqli_fetch_array($stmt)) { ?>
                            <li class="product">
